...
//const decoderAscii = new TextDecoder("latin1");
...
class AsyncIterReader extends BaseAsyncIterReader {
  ...
  async readtoCRLFCRLF2() {
    ...
    for await (let chunk of this) {
      let string = decoderAscii.decode(chunk);
      if (string.length !== chunk.byteLength) {
        throw new Error("size mismatch");
      }

      if (checkRem) {
        if (string.startsWith(checkRem)) {
          inx = checkRem.length;
          lastString = string;
          lastChunk = chunk;
          break;
    ...
    if (lastString) {
      const first = lastString.slice(0, inx + 1);
      strings.push(first);
      size += first.length;

      const remainder = lastChunk.slice(inx + 1);
  ...
  async readlineRaw(maxLength) {
    ...
    for await (let chunk of this) {
      if (maxLength && (size + chunk.byteLength) > maxLength) {
        ...
        const chunkSlice = chunk.slice(0, inx + 1);
        const lineInx = chunkSlice.indexOf(10);
...
class LimitReader extends BaseAsyncIterReader
{
  ...
  async readlineRaw(maxLength, doubleLine = false) {
    if (this.limit <= 0) {
      return null;
    }

    const result = await this.sourceIter.readlineRaw(maxLength ? Math.min(maxLength, this.limit) : this.limit, doubleLine);
